...
@Component({
  selector: 'app-audit-trail-by-billing',
  templateUrl: './audit-trail-by-billing.component.html',
  styleUrls: ['./audit-trail-by-billing.component.scss']
})
@PageConfiguration({
  trackInit: true,
...
export class AuditTrailByBillingComponent extends NgcPage implements OnInit {
  ...
  onSearch() {
    ...
    if (audit.toDate !== null && audit.fromDate !== null) {
      if (audit.toDate < audit.fromDate) {
    ...
    audit.serviceNumber = this.auditForm.get('serviceNumber').value;
    audit.pdNumber = this.auditForm.get('pdNumber').value;
    audit.customerCode = this.auditForm.get('customerCode').value;
    audit.eventType = this.auditForm.get('eventType').value;
    audit.hawbNumber = this.auditForm.get('hawbNumber').value;
    if (audit.shipmentNumber == null && audit.serviceNumber == null && audit.pdNumber == null && audit.customerCode == null) {
      this.flagToDisplayData = false;
      this.showErrorMessage('error.enter.shpmnt.service.cust.code.pd');
      return;
    } else {
      this.onClearMessage;
    }

    if (audit.customerCode != null) {

      if ((audit.fromDate === null || audit.fromDate === '')
        || (audit.toDate === null || audit.toDate === '')) {
        this.flagToDisplayData = false;
        this.showErrorMessage('error.from.to.date.required');
        return;
      } else {
        this.onClearMessage;
      }
    ...
    this.auditService.onBillingAuditSearch(audit).subscribe(response => {
      ...
      if (response.data.auditList) {
        ...
        auditListData.forEach(element => {
          element.sno = index++;
          let dataNeedToShow = 'BILLING' + ' | ' + this.dateFormated(element.eventDateTime);
          ...
          if (element.eventValue) {
            ...
            classAttributes.forEach(element1 => {
              ...
              element.eventValue = element.eventValue.replace(eventValueObj.changedValue, (eventValueObj.changedValue.bold()).fontcolor("blue"));
  ...
  onCompanyLOVSelect(object) {
    this.auditForm.get('customerCode').setValue(object.code);
    if (this.auditForm.get('customerCode').value != null) {
      this.auditForm.get('fromDate').setValidators([Validators.required]);
      this.auditForm.get('toDate').setValidators([Validators.required]);
    } else {
      this.auditForm.get('fromDate').clearValidators();
      this.auditForm.get('toDate').clearValidators();
    }
  }
