...
@Component({
  selector: 'app-audit-trail-by-awb',
  templateUrl: './audit-trail-by-awb.component.html',
  styleUrls: ['./audit-trail-by-awb.component.scss']
})
@PageConfiguration({
  trackInit: true,
...
export class AuditTrailByAwbComponent extends NgcPage implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log(this.shipmentNumberData);
    if (this.shipmentNumberData) {
      this.auditForm.get('shipmentNumber').setValue(this.shipmentNumberData);
      console.log(this.auditForm.get('shipmentNumber').value);
      this.onSearch();
    }
  }
  onSearch() {
    ...
    this.auditService.onAWBAuditSearch(audit).subscribe(response => {
      ...
      if (response.data.auditList) {
        ...
        auditListData.forEach(element => {
          element.sno = index++;
          let dataNeedToShow = 'AWB' + ' | ' + this.dateFormated(element.eventDateTime);
