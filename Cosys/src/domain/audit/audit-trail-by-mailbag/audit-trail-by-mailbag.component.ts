...
@Component({
  selector: 'app-audit-trail-by-mailbag',
  templateUrl: './audit-trail-by-mailbag.component.html',
  styleUrls: ['./audit-trail-by-mailbag.component.scss']
})
@PageConfiguration({
  trackInit: true,
...
export class AuditTrailByMailbagComponent extends NgcPage implements OnInit {
  ...
  onSearch() {
    ...
    audit.entityType = 'MailBag';
    audit.entityValue = this.auditForm.get('mailbagNumber').value;
    ...
    audit.dnNumber = this.auditForm.get('dnNumber').value;

    // if (audit.mailbagNumber === null
    // ) {
    //   this.showErrorMessage('Please Enter MailBag Number to Search');
    //   return;
    // } else {
    //   this.onClearMessage;
    // }
    this.auditService.onMailBagAuditSearch(audit).subscribe(response => {
      ...
      if (response.data.auditList) {
        ...
        auditListData.forEach(element => {
          element.sno = index++;
          let dataNeedToShow = 'MAILBAG' + ' | ' + this.dateFormated(element.eventDateTime);
