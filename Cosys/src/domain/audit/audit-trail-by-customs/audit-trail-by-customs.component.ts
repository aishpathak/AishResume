...
@Component({
  selector: 'app-audit-trail-by-customs',
  templateUrl: './audit-trail-by-customs.component.html',
  styleUrls: ['./audit-trail-by-customs.component.scss']
})
@PageConfiguration({
  trackInit: true,
  callNgOnInitOnClear: true
})
export class AuditTrailByCustomsComponent extends NgcPage implements OnInit {
  ...
  onSearch() {
    ...
    if ((audit.flightKey === null || audit.flightKey === '')
      && audit.flightDate === null
      && audit.shipmentNumber === null || audit.shipmentNumber === ''
    ) {
    ...
    this.auditService.getAuditTrailByCustoms(audit).subscribe(response => {
