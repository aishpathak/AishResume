...
@Component({
  selector: 'app-audit-trail-by-userrole',
  templateUrl: './audit-trail-by-userrole.component.html',
  styleUrls: ['./audit-trail-by-userrole.component.scss']
})
@PageConfiguration({
  trackInit: true,
  callNgOnInitOnClear: true
})
export class AuditTrailByUserroleComponent extends NgcPage implements OnInit {
  ...
  onSearch() {
    ...
    audit.roleCode = this.auditForm.get('roleCode').value;
    audit.eventType = this.auditForm.get('eventType').value;
    if ((audit.user === null || audit.user === '')
      && audit.roleCode === null    
    ) {
      this.showErrorMessage('error.provide.user.role.code');
      return;
    } else {
      this.onClearMessage;
    }
    
    this.auditService.getAuditTrailByUserRole(audit).subscribe(response => {
