...
@Component({
  selector: 'app-audit-trail-by-equipment',
  templateUrl: './audit-trail-by-equipment.component.html',
  styleUrls: ['./audit-trail-by-equipment.component.scss']
})
@PageConfiguration({
  trackInit: true,
...
export class AuditTrailByEquipmentComponent extends NgcPage implements OnInit {
  ...
  onSearch() {
    ...
    this.auditService.onEquipmentAuditSearch(audit).subscribe(response => {
      ...
      if (response.data.auditList) {
        ...
        auditListData.forEach(element => {
          element.sno = index++;
          let dataNeedToShow = 'EQUIPMENT' + ' | ' + this.dateFormated(element.eventDateTime);
