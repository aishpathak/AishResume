...
@Component({
    selector: 'ngc-manage-cargo-trucking-flight',
    templateUrl: './manage-cargo-trucking-flight.component.html',
    styleUrls: ['./manage-cargo-trucking-flight.component.scss']
})

@PageConfiguration({
    trackInit: true
})
export class ManageCargoTruckingFlightComponent extends NgcPage {
    ...
    ngOnInit() {
        ...
        this._acceptanceService.onEditEAcceptance(this.dataRetrievedRequest).subscribe(response => {
            if (response.data != null && response.data.ruleExecutionDetails != null && response.data.ruleExecutionDetails.length > 0) {
    ...
    subscribeLocalAuthority(event) {
        if (event.desc === 'Permit To Follow') {
            this.permitNumberFlag = false;
            this.excemptionCodeFlag = false;
            this.permitToFollowFlag = true;
            this.form.get(['localAuthority', 'type']).setValue('PTF', { onlySelf: true, emitEvent: false });
            ...
            this.form.get(['localAuthority', 'details', 0, 'permitNumbers']).clearValidators();
        } else if (event.desc === 'Exempt') {
            this.permitNumberFlag = false;
            this.excemptionCodeFlag = true;
            this.permitToFollowFlag = false;
            this.onAutoDeletePermitNumberRows();
            this.form.get(['localAuthority', 'details', 0, 'permitNumbers']).clearValidators();
        } else if (event.desc === 'Permit Number') {
            this.permitNumberFlag = true;
            this.excemptionCodeFlag = false;
            this.permitToFollowFlag = false;
            this.form.get(['localAuthority', 'details', 0, 'permitNumbers']).setValidators([Validators.required]);
            this.onAutoDeletePermitNumberRows();
        }
    }
    ...
    onSave() {
        ...
        if (NgcUtility.isTenantCityOrAirport(this.form.get('shipment').get('origin').value)) {
            this.showErrorMessage('exp.accpt.truckflight.originnotsin');
            return;
        }
    ...
    onClickAwbDropDown(event) {
        ...
        this._acceptanceService.onEditEAcceptance(this.dataRetrievedRequest).subscribe(response => {
            ...
            if (!response.messageList) {
                ...
                responseData.acceptanceService.incomingFlightDate = this.dataRetrievedFromManageCargoAcceptance.incomingFlightDate;

                responseData.acceptanceService.incomingFlightCarrier = this.dataRetrievedFromManageCargoAcceptance.incomingFlightCarrier;


                if (this.dataRetrievedFromManageCargoAcceptance.incomingFlightCarrier != null && this.dataRetrievedFromManageCargoAcceptance.incomingFlightNumber != null) {
                    let flightKey: any = this.dataRetrievedFromManageCargoAcceptance.incomingFlightCarrier + this.dataRetrievedFromManageCargoAcceptance.incomingFlightNumber;
                    responseData.acceptanceService.incomingFlight = flightKey;
                } else {
                    responseData.acceptanceService.incomingFlight = null;
                }
    ...
    callExportChargeMandatory(schGroup: any) {
        ...
        if (schGroup !== null) {
            setTimeout(() => {
                this.retrieveDropDownListRecords('SHC_CODE_WH').subscribe(data => {
                    const billingShc = data;
                    if (billingShc.length > 0) {
                        billingShc.forEach(element => {
                            if (schGroup.length > 0) {
