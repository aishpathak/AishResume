...
@Component({
  ...
  templateUrl: './export-FlightEVM-DashboardTV.component.html',
  styleUrls: ['./export-FlightEVM-DashboardTV.component.scss'],
...
export class ExportFlightEVMDashboardTVComponent extends NgcPage implements OnInit {
  dateTo: any;
  showTableFlag: any;
  ...
  public fetchExportFlightInfo() {
    ...
    this.userService.getSlaDashBoardTVExport(flightDates).subscribe(data => {
      ...
      if (!this.showResponseErrorMessages(data)) {
        if (data.data.exportList && data.data.exportList.length > 0) {
          ...
          console.log("current DashboardExportBatchLogId: " + data.data.dashboardBatchLogId);//doubt
  ...
  */
  ...
  public checkLatestRecordsExport() {
    if (null != this.dashboardBatchLogId) {
      ...
      this.userService.getLatestDboardRecExport(dboardBatchLog).subscribe(responseBean => {
  ...
  public cellsRenderer = (row: number, column: string, value: any, rowData: any): string => {
    ...
    else if (column === 'svcRptFinalSlaColor') {
      if ("AMBER" === rowData.svcRptFinalSlaColor) {
        rowData.svcRptFinalSlaColor = "YELLOW";
      }
      return `
            <div>
                <svg height="20" width="20">
                  <circle cx="10" cy="10" r="10" style = "fill : ${rowData.svcRptFinalSlaColor} " />
                </svg>
            </div>
            `
    }
