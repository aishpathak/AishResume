...
@Component({
  ...
  templateUrl: './import-FlightEVM-SlaTV.component.html',
  styleUrls: ['./import-FlightEVM-SlaTV.component.scss']
})

export class ImportFlightEVMSlaTVComponent extends NgcPage implements OnInit, OnDestroy {
  responseArray: any[];
  returnResult: any[];
  refreshTime: any;
  recordsPerPage: any;
  ...
  getLatestDboardRecImportSub: Subscription;
  nextFireTime: Date;
  lastJobRun: any;
  nextJobRun: any;
  ...
  ngOnInit() {
    ...
    // to run every 30 sec to check latest Import records
  ...
  onDashBoardPageLoad1() {
    ...
    this.getDataOnDashboardPageLoad = this.eventsService.getSlaTVImport(this.flightDates).subscribe(responseBean => {
      if (responseBean.data) {
        ...
        this.responseArray = responseBean.data.importList;
  ...
  reset() {
    this.router.navigate(["events/ImportFlightEVMSlaTV"]);
    //this.router.navigate(["Import/cdh/dashboardtv"]);
  }
...
*/
  public checkLatestRecordsImport() {
    if (null != this.dashboardBatchLogId) {
      ...
      this.getLatestDboardRecImportSub = this.eventsService.getLatestDboardRecImport(dboardBatchLog).subscribe(responseBean => {
