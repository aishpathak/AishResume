...
@Component({
  selector: 'app-audit-trail-by-agent',
  templateUrl: './audit-trail-by-agent.component.html',
  styleUrls: ['./audit-trail-by-agent.component.scss']
})
@PageConfiguration({
  trackInit: true,
...
export class AuditTrailByAgentComponent extends NgcPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.retrieveDropDownListRecords('AUDIT_ENTITY_TYPE', 'query', null)
      .subscribe(value => {
        value.forEach(entity => {
          if (entity.code == 'AGENT') {
            this.mandatoryFields = entity.booleanParameter2;
          }
        });
      })
  ...
  onSearch() {
    ...
    audit.customerName = this.auditForm.get('customerName').value;
    audit.customerCode = this.auditForm.get('customerCode').value;
    audit.eventType = this.auditForm.get('eventType').value;
    audit.uenNumber = this.auditForm.get('uenNumber').value;
    audit.loginId = this.auditForm.get('loginId').value;

    if ((audit.fromDate === null || audit.fromDate === '')
      && audit.toDate === null ||
      audit.eventType === null ||
      audit.customerCode == null
    ) {
      this.showErrorMessage('uld.mandatory.field.must.not.be.empty');
      return;
    } else {
      this.onClearMessage;
    }

    this.auditService.onAgentAuditSearch(audit).subscribe(response => {
      ...
      if (response.data.auditList) {
        ...
        auditListData.forEach(element => {
          element.sno = index++;
          let dataNeedToShow = 'AGENT' + ' | ' + this.dateFormated(element.eventDateTime);
  ...
  onCompanyLOVSelect(object) {
    this.auditForm.get('customerName').setValue(object.desc);
  }
