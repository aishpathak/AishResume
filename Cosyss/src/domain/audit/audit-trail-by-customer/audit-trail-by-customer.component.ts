...
@Component({
  selector: 'app-audit-trail-by-customer',
  templateUrl: './audit-trail-by-customer.component.html',
  styleUrls: ['./audit-trail-by-customer.component.scss']
})
@PageConfiguration({
  trackInit: true,
  callNgOnInitOnClear: true
})
export class AuditTrailByCustomerComponent extends NgcPage implements OnInit {
  ...
  onSearch() {
    ...
    if (!audit.customerCode && !audit.uenNumber && !audit.loginId) {
      this.showErrorMessage('error.cust.code.uen.no.login.id.required');
      return;
    }
    ...
    this.auditService.getAuditTrailByCustomer(audit).subscribe(response => {
