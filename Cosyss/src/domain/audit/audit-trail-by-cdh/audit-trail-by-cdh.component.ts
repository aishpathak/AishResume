...
@Component({
  selector: 'app-audit-trail-by-cdh',
  templateUrl: './audit-trail-by-cdh.component.html',
  styleUrls: ['./audit-trail-by-cdh.component.scss']
})
@PageConfiguration({
  trackInit: true,
...
export class AuditTrailByCdhComponent extends NgcPage implements OnInit {
  ...
  onSearch() {
    ...
    if (audit.shipmentNumber === null && (audit.flightKey === null || audit.flightDate === null)
    ) {
      this.showErrorMessage('error.shpmnt.flight.details.required');
      return;
    } else {
      this.onClearMessage;
    }
    this.auditService.getAuditTrailByCDH(audit).subscribe(response => {
