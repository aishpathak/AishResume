...
@Component({
  selector: 'app-audit-trail-by-tracing',
  templateUrl: './audit-trail-by-tracing.component.html',
  styleUrls: ['./audit-trail-by-tracing.component.scss']
})
@PageConfiguration({
  trackInit: true,
  callNgOnInitOnClear: true
})
export class AuditTrailByTracingComponent extends NgcPage implements OnInit {
  ...
  onSearch() {
    ...
    if ((audit.fromDate === null || audit.fromDate === '')
      && audit.toDate === null
      && (audit.entityValue === null || audit.entityValue === '')
    ) {
    ...
    this.auditService.getAuditTrailByTracing(audit).subscribe(response => {
