...
@Component({
  selector: 'app-audit-trail-by-efacilitation',
  templateUrl: './audit-trail-by-efacilitation.component.html',
  styleUrls: ['./audit-trail-by-efacilitation.component.scss']
})
@PageConfiguration({
  trackInit: true,
...
export class AuditTrailByEfacilitationComponent extends NgcPage implements OnInit {
  ...
  onSearch() {
    ...
    if ((audit.fromDate === null || audit.fromDate === '')
      ...
      this.showErrorMessage('error.date.range.mandatory');
      return;
    } else {
      this.onClearMessage;
    }
    this.auditService.onEFacilitationAuditSearch(audit).subscribe(response => {
      ...
      if (response.data.auditList) {
        ...
        auditListData.forEach(element => {
          element.sno = index++;
          let dataNeedToShow = 'EFACILITATION' + ' | ' + this.dateFormated(element.eventDateTime);
        ...
        if (audit.entityType === 'EFACILITATION') {
