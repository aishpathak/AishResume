...
@Component({
  selector: 'app-add-exp-fbl',
  templateUrl: './add-exp-fbl.component.html',
  styleUrls: ['./add-exp-fbl.component.scss']
})
@PageConfiguration({
  trackInit: true,
  callNgOnInitOnClear: true
})
export class AddExpFblComponent extends NgcPage {
  ...
  sendRequestAddShipment() {
    ...
    //                 this.navigateTo(this.router, '/export/expfbl', this.response);
    //               }
    //             });
  ...
  onCancel() {
    this.navigateTo(this.router, '/export/expfbl', this.MultipleShipmentForm.getRawValue());
  }
    ...
    if (searchRequest.shipmentNumber) {
      this.exportService.getShipmentMasterData(searchRequest).subscribe(resp => {
        ...
        if (resp.success) {
          ...
          this.MultipleShipmentForm.get(['MultipleShipmentBooking', i]).get('bookingPriority').setValue(resp.data.bookingPriority);
          this.MultipleShipmentForm.get(['MultipleShipmentBooking', i]).get('buInstruction').setValue(resp.data.buInstruction);
          this.MultipleShipmentForm.get(['MultipleShipmentBooking', i]).get('fblRemarks').setValue(resp.data.fblRemarks);
          this.MultipleShipmentForm.get(['MultipleShipmentBooking', i]).get('shipmentCode').setValue(resp.data.shipmentCode);
