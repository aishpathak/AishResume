...
@Component({
  selector: 'ngc-manage-cargo-trucking-surf',
  templateUrl: './manage-cargo-trucking-surf.component.html',
  styleUrls: ['./manage-cargo-trucking-surf.component.scss']
})

@PageConfiguration({
  trackInit: true
})
export class ManageCargoTruckingSurfComponent extends NgcPage {
  ...
  onSave() {
    ...
    if (NgcUtility.isTenantCityOrAirport(this.form.get('shipment').get('origin').value)) {
      this.showErrorMessage('exp.accpt.trucksurf.originnotsin');
      return;
    }
    ...
    this.retrieveDropDownListRecords('Acceptance$LodgeInTime').subscribe(data => {
      ...
      if (differnceInTime > 0 && NgcUtility.isTenantCityOrAirport(eAcceptanceData.shipment.origin)) {
        if (differnceInTime < this.lateLodgeInTime) {
          ...
          if (Math.abs(differnceInTime) > this.earlyLodgeInTime) {
