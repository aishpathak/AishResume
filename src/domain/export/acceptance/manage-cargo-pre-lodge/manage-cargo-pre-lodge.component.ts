...
import { Component, OnInit, NgZone, ElementRef, ViewChild, ViewContainerRef, OnDestroy, ViewChildren } from '@angular/core';
import {
  NgcPage, NgcFormGroup, NgcFormArray, NgcFormControl, NgcWindowComponent, PageConfiguration, NgcLOVComponent, NgcUtility
} from 'ngc-framework';
...
@Component({
  selector: 'app-manage-cargo-pre-lodge',
  templateUrl: './manage-cargo-pre-lodge.component.html',
  styleUrls: ['./manage-cargo-pre-lodge.component.scss']
})
...
export class ManageCargoPreLodgeComponent extends NgcPage {
  ...
  ngOnInit() {
    ...
    this._acceptanceService.onEditEAcceptance(this.dataRetrievedRequest).subscribe(response => {
      ...
      if (!response.messageList) {
        ...
        //this.form.get(['localAuthority', 'details', 0, 'appointedAgent']).setValue(/^\d+$/.test(this.dataRetrievedFromManageCargoAcceptance.agentCode) ? 'EXX' : this.dataRetrievedFromManageCargoAcceptance.agentCode);
        if (
    ...
    this.form.get(['localAuthority', 'type']).valueChanges.subscribe(
      (newValue) => {
        if (newValue === 'PTF') {
          ...
          this.form.get(['localAuthority', 'details', 0, 'customsAgentCode']).patchValue('PTF');
          this.form.get(['localAuthority', 'details', 0, 'appointedAgent']).patchValue(/^\d+$/.test(this.dataRetrievedFromManageCargoAcceptance.agentCode) ? 'EXX' : this.dataRetrievedFromManageCargoAcceptance.agentCode);
        }
      });
  }
