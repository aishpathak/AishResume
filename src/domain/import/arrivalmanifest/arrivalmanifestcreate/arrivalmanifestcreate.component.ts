...
@Component({
	selector: 'app-arrivalmanifestcreate',
	moduleId: module.id,
	templateUrl: 'arrivalmanifestcreate.component.html',
	styleUrls: ['arrivalmanifestcreate.component.css']
})
...
export class ArrivalmanifestcreateComponent extends NgcPage {
	shipmentData: any;
	private _list: any;
	response: any;
	responseForSave: any;
	...
	@Input('list')
	public set list(list: ArrivalManifestData) {
		this._list = list;
		if (list != null) {
			this.arrivalFlightDetails.get('searchArrivalData.flight').setValue(list.flightNumber);
			this.arrivalFlightDetails.get('searchArrivalData.date').setValue(list.flightDate);
		}
		this.onSearch();
	}
	...
	public onSearch(): void {
		...
		this.importService.fetchArrivalSearchDetails(flightSearchData).subscribe(data => {
			...
			if (data.data) {
				...
				this.responseForSave = data.data;
				//console.log(this.responseForSave);
				...
				this.response.segments.forEach(element => {
					...
					element.manifestedUlds = [];
					element.bulkShipments = [];
				});
				//re-setting arrays
				this.existingbulkShipments = [];
				this.existingManifestedUlds = [];
				this.arrivalFlightDetails.get('segments').patchValue(this.response.segments);
	...
	public onAddingShipment(event, parentIndex, childIndex) {
		this.list.segments.forEach(element => {
